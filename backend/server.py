...
BOT_USERNAME = os.environ.get('BOT_USERNAME', 'SlitInfo_bot')

# Create the main app
app = FastAPI(title="SlitInfo - Telegram Bot API")
...
def normalize_phone_number(phone: str) -> str:
    ...
    if clean_phone.startswith('+7'):
        clean_phone = '8' + clean_phone[2:]
    
    # Если номер начинается с 7 (без +), заменяем на 8
    elif clean_phone.startswith('7') and len(clean_phone) == 11:
        clean_phone = '8' + clean_phone[1:]
    
    # Если номер из 10 цифр, добавляем 8
...
def detect_search_type(query: str) -> str:
    ...
    if re.match(r'^8\d{10}$', normalized_phone):
        return "📱 Телефон"
...
def create_subscription_keyboard():
    ...
    return {
        "inline_keyboard": [
            [
                {"text": "📢 Подписаться на канал", "url": "https://t.me/SlitInfo"}
            ],
            [
...
def escape_markdown(text: str) -> str:
    ...
    # Only escape characters that actually break Markdown parsing
    # For regular text, we only need to escape: * _ ` [
    text = text.replace('*', '\\*')
...
def format_search_results(results: Dict[str, Any], query: str, search_type: str) -> str:
    ...
    if results.get('status') == 'error':
        error_msg = results.get('error', {}).get('message', 'Неизвестная ошибка')
        return f"❌ *Ошибка:* {escape_markdown(error_msg)}"
    ...
    if 'items' in data and isinstance(data['items'], list):
        ...
        for i, source_data in enumerate(data['items'][:5], 1):
            if 'source' in source_data and 'hits' in source_data:
                ...
                database_name = source.get('database', 'N/A')
                db_display = db_names.get(database_name, f"📊 {database_name}")
                
                formatted_text += f"*{i}. {db_display}*\n"
                formatted_text += f"📁 База: {escape_markdown(source.get('collection', 'N/A'))}\n"
                    ...
                    for item in hits['items'][:2]:
                        for key, value in item.items():
                            ...
                            if key in ['phone', 'телефон', 'tel', 'mobile']:
                                ...
                                gender_value = gender_map.get(str(value), value)
                                formatted_text += f"⚥ {escape_markdown(str(gender_value))}\n"
...
@api_router.get("/")
async def root():
    return {"message": "SlitInfo - Telegram Bot API", "status": "running"}
...
async def handle_cryptobot_payment(webhook_data: Dict[str, Any]):
    ...
    try:
        ...
        if update_type == 'invoice_paid':
            ...
            # Parse user_id from description like "Пополнение баланса SlitInfo для пользователя 123456789"
            import re
            ...
            if status == 'paid':
                ...
                if result.modified_count > 0:
                    ...
                    admin_notification = f"💰 *НОВОЕ ПОПОЛНЕНИЕ*\n\n"
                    admin_notification += f"👤 *Пользователь:* {user_id}\n"
                    admin_notification += f"💳 *Сумма:* {amount}₽\n"
                    admin_notification += f"🤖 *Метод:* Криптобот\n"
                    admin_notification += f"📋 *ID платежа:* {invoice_id}\n"
                    admin_notification += f"📅 *Время:* {datetime.utcnow().strftime('%d.%m.%Y %H:%M:%S')}"
...
async def handle_subscription_check(chat_id: int, user_id: int):
    ...
    if is_subscribed:
        ...
        await send_telegram_message(
            chat_id,
            "❌ *Подписка не найдена*\n\n📢 Подпишитесь на канал @SlitInfo и попробуйте снова",
...
async def show_referral_menu(chat_id: int, user: User):
    ...
    referral_text += f"📋 *Условие:* реферал должен подписаться на @SlitInfo\n\n"
    ...
    referral_text += f"3. Друг подписывается на @SlitInfo\n"
...
async def show_help_menu(chat_id: int, user: User):
    ...
    help_text += f"SlitInfo помогает найти информацию о людях из открытых источников интернета.\n\n"
...
async def handle_admin_callback(chat_id: int, user: User, data: str):
    ...
    if data == "admin_panel":
        ...
        admin_text += f"🔧 Управление сервисом SlitInfo\n\n"
        ...
        # Подсчет прибыли ТОЛЬКО из пополнений (не из трат на поиски)
...
async def handle_stars_payment(chat_id: int, user: User, amount: str):
    ...
    try:
        ...
        invoice_data = {
            ...
            "description": f"Пополнение баланса сервиса SlitInfo через Telegram Stars",
...
async def handle_successful_payment(message: Dict[str, Any]):
    ...
    try:
        ...
        if len(parts) >= 4 and parts[0] == 'stars' and parts[1] == 'payment':
            ...
            admin_notification += f"👤 *Пользователь:* {user_id}\n"
            admin_notification += f"💳 *Сумма:* {amount}₽\n"
            admin_notification += f"⭐ *Метод:* Telegram Stars\n"
            admin_notification += f"📋 *ID платежа:* {payment_info.get('telegram_payment_charge_id')}\n"
            admin_notification += f"📅 *Время:* {datetime.utcnow().strftime('%d.%m.%Y %H:%M:%S')}"
    ...
    if text.startswith('/start'):
        ...
        if not user.is_admin:
            ...
            if not is_subscribed:
                await send_telegram_message(
                    chat_id,
                    f"🎯 *ДОБРО ПОЖАЛОВАТЬ В SlitInfo!*\n\n🔒 *Для использования сервиса подпишитесь на канал @SlitInfo*",
                    reply_markup=create_subscription_keyboard()
                )
                return
    ...
    # Handle admin balance commands
        ...
        if len(parts) == 2:
            try:
                ...
                if result.modified_count > 0:
                    # Создаем запись о платеже для учета
                    payment = Payment(
                        user_id=target_id,
                        amount=amount,
                        payment_type="admin",
                        payment_id=f"admin_{user.telegram_id}_{datetime.utcnow().timestamp()}",
                        status="completed"
                    )
                    await db.payments.insert_one(payment.dict())
                    ...
                    # Уведомление админу о начислении (для записи)
                    admin_notification = f"💰 *НАЧИСЛЕНИЕ БАЛАНСА*\n\n"
                    admin_notification += f"👤 *Пользователь:* {target_id}\n"
                    admin_notification += f"💳 *Сумма:* {amount}₽\n"
                    admin_notification += f"👑 *Метод:* Админ ({user.username or user.first_name})\n"
                    admin_notification += f"📅 *Время:* {datetime.utcnow().strftime('%d.%m.%Y %H:%M:%S')}"
                    
                    # Отправляем уведомление только если это не сам админ
...
async def handle_search_query(chat_id: int, query: str, user: User):
    ...
    if not user.is_admin:
        ...
        if not is_subscribed:
            await send_telegram_message(
                chat_id,
                "🔒 Для поиска нужна подписка на @SlitInfo",
                reply_markup=create_subscription_keyboard()
            )
            return
    ...
    # Если это телефон, нормализуем его для поиска
    search_query = query
    if search_type == "📱 Телефон":
        search_query = normalize_phone_number(query)
        logging.info(f"📱 Нормализация телефона: '{query}' -> '{search_query}'")
    ...
    try:
        results = await usersbox_request("/search", {"q": search_query})
...
async def create_cryptobot_invoice(amount: float, user_id: int, currency: str = "RUB") -> Dict[str, Any]:
    ...
    try:
        ...
        payload = {
            ...
            "fiat": currency,
            "amount": str(amount),
            "description": f"Пополнение баланса SlitInfo для пользователя {user_id} на {amount}₽",
            "paid_btn_name": "callback",
            "paid_btn_url": "https://t.me/SlitInfo_bot",
